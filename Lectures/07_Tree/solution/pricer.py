...
class TreePricer(Pricer):
    ...
    def pre_final_value(self, spot: float, step: int, discounted_continuation_value: float) -> float:
    ...
    def calc_fair_value(self) -> float:
        ...
        for step in range(self._params.nr_steps - 1, -1, -1):
            for i in range(len(spot_tree[step])):
                ...
                continuation_value_tree[step][i] = self.pre_final_value(spot, step, discounted_continuation_value)
...
class AmericanTreePricer(TreePricer):
    ...
    def pre_final_value(self, spot: float, step: int, discounted_continuation_value: float) -> float:
        ...
        return max(discounted_continuation_value,intrinsic_value) if self._contract.get_long_short() == LongShort.LONG \
