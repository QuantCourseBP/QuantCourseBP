...
class EuropeanDigitalAnalyticPricer(Pricer):
    ...
    def __init__(self, contract: EuropeanDigitalContract, model: MarketModel, params: Params) -> None:
        if not isinstance(contract, EuropeanDigitalContract):
            raise TypeError(
                f'Contract must be EuropeanDigitalContract but received {type(contract).__name__}'
            )
        super().__init__(contract, model, params)
    ...
    def calc_fair_value(self) -> float:
        ...
        d2 = EuropeanAnalyticPricer.calc_d2(
        spot / strike, vol, rate, time_to_expiry)
    ...
    def calc_gamma(self) -> float:
        ...
        T = self.contract.expiry - self.valuation_time

        sigma = self.model.vol
        r = self.model.risk_free_rate

        df = self.model.calc_df(T)
        d2 = EuropeanAnalyticPricer.calc_d2(S / K, sigma, r, T)

        phi = norm.pdf(d2)

        gamma = -df * phi * (d2 + sigma * np.sqrt(T)) / (S**2 * sigma**2 * T)

        return self.contract.direction * gamma
    ...
    def calc_vega(self) -> float:
        ...
        vega = -df * norm.pdf(d2) * d2 / sigma

        return self.contract.direction * vega
    ...
    def calc_theta(self) -> float:
        ...
        theta = df * (
            -S * norm.pdf(d2) * sigma / (2 * np.sqrt(T))
            + r * norm.cdf(d2)
        )

        return self.contract.direction * theta
    def calc_rho(self) -> float:
        S = self.model.spot
        K = self.contract.strike
        T = self.contract.expiry - self.valuation_time
        ...
        rho = -T * df * norm.cdf(d2)

        return self.contract.direction * rho