...
class Pricer(ABC):
    ...
    def calc_gamma(self, method: GreekMethod) -> float:
        ...
        if self.bumpsizemethod == BumpSizeMethod.ABSOLUTE:
            ...
            bump_size = self.relative_bump_size * self.model.spot
        V0 = self.calc_fair_value()
        if self.finitemethod == FiniteMethod.CENTRAL:
            shifts = (bump_size, -bump_size)
        elif self.finitemethod == FiniteMethod.FORWARD:
            shifts = (2 * bump_size, bump_size)
        elif self.finitemethod == FiniteMethod.BACKWARD:
            shifts = (-bump_size, -2 * bump_size)
        else:
            raise ValueError("Not supported FiniteMethod")
        bumped_fair_values = list()
        for b in shifts:
            model = copy.deepcopy(self.model)
            model.bump_spot(b)
        ...
        if self.finitemethod == FiniteMethod.CENTRAL:
            ...
            return (bumped_fair_values[0] - 2 * V0 + bumped_fair_values[1]) / (bump_size ** 2)

        elif self.finitemethod == FiniteMethod.FORWARD:
            
            return (bumped_fair_values[0] - 2 * bumped_fair_values[1] + V0) / (bump_size ** 2)

        elif self.finitemethod == FiniteMethod.BACKWARD:
            
            return (V0 - 2 * bumped_fair_values[0] + bumped_fair_values[1]) / (bump_size ** 2)
