...
# HW
# very similar to EuropeanContract ~ change only in payoff
# (based on: https://thetradinganalyst.com/digital-option/, and 
#            https://en.wikipedia.org/wiki/Black%E2%80%93Scholes_model, and
#            https://quantpie.co.uk/bsm_bin_c_formula/bs_bin_c_summary.php) 
#
# earnings will be set by a percentage of the strike
# can be something else, does not really matter...
class EuropeanDigitalAnalyticPricer(Pricer):
    ...
    def calc_fair_value(self) -> float:
        ...
        payout_percentage = self.contract.payout_percentage
        if self.contract.derivative_type == PutCallFwd.CALL:
            return direction * payout_percentage * strike * df * norm.cdf(d2)
        elif self.contract.derivative_type == PutCallFwd.PUT:
            return direction * payout_percentage * strike * df * norm.cdf(-d2)
        else:
            self.contract.raise_incorrect_derivative_type_error()
    ...
    def calc_delta(self) -> float:
        ...
        payout_percentage = self.contract.payout_percentage
        if self.contract.derivative_type == PutCallFwd.CALL:
            greek = payout_percentage * strike * df * norm.pdf(d2) / spot / vol / np.sqrt(time_to_expiry)
        elif self.contract.derivative_type == PutCallFwd.PUT:
            greek = -payout_percentage * strike * df * norm.pdf(d2) / spot / vol / np.sqrt(time_to_expiry)
        else:
            self.contract.raise_incorrect_derivative_type_error()
    ...
    def calc_gamma(self) -> float:
        ...
        payout_percentage = self.contract.payout_percentage
        if self.contract.derivative_type == PutCallFwd.CALL:
            greek = -payout_percentage * strike * df * norm.pdf(d2) * d1 / spot ** 2 / vol ** 2 / time_to_expiry
        elif self.contract.derivative_type == PutCallFwd.PUT:
            greek = payout_percentage * strike * df * norm.pdf(d2) * d1 / spot ** 2 / vol ** 2 / time_to_expiry
        else:
            self.contract.raise_incorrect_derivative_type_error()
    ...
    def calc_vega(self) -> float:
        ...
        payout_percentage = self.contract.payout_percentage
        if self.contract.derivative_type == PutCallFwd.CALL:
            greek = -payout_percentage * strike * df * norm.pdf(d2) * d1 / vol
        elif self.contract.derivative_type == PutCallFwd.PUT:
            greek = payout_percentage * strike * df * norm.pdf(d2) * d1 / vol
        else:
            self.contract.raise_incorrect_derivative_type_error()
    ...
    def calc_theta(self) -> float:
        ...
        payout_percentage = self.contract.payout_percentage
        if self.contract.derivative_type == PutCallFwd.CALL:
            greek = payout_percentage * strike * df * (norm.pdf(d2) / (2. * time_to_expiry * vol * np.sqrt(time_to_expiry)) * 
                                                                       (np.log(spot / strike) - (rate - vol ** 2 / 2.) * time_to_expiry)
                                                       + rate * norm.cdf(d2))
        elif self.contract.derivative_type == PutCallFwd.PUT:
            greek = payout_percentage * strike * df * (-norm.pdf(d2) / (2. * time_to_expiry * vol * np.sqrt(time_to_expiry)) * 
                                                                       (np.log(spot / strike) - (rate - vol ** 2 / 2.) * time_to_expiry)
    ...
    def calc_rho(self) -> float:
        ...
        payout_percentage = self.contract.payout_percentage
        if self.contract.derivative_type == PutCallFwd.CALL:
            greek = payout_percentage * strike * df * (np.sqrt(time_to_expiry) / vol * norm.pdf(d2) - time_to_expiry * norm.cdf(d2))
