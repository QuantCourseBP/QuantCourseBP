...
class Pricer(ABC):
    ...
    absolute_bump_size: float = 0.01
    bump_type: BumpType = BumpType.RELATIVE
    fd_method: FiniteDifferenceMethod = FiniteDifferenceMethod.CENTRAL
    ...
    def calc_delta(self, method: GreekMethod) -> float:
        ...
        if self.bump_type == BumpType.RELATIVE:
            ...
            bump_size = self.absolute_bump_size
        else:
            raise ValueError("Unknown bump type!")
        if self.fd_method == FiniteDifferenceMethod.FORWARD:
            model_for = copy.deepcopy(self.model)
            model_for.bump_spot(bump_size)
            bumped_pricer_for = self.create_pricer(self.contract, model_for, self.params)
            bumped_fair_value_for = (bumped_pricer_for.calc_fair_value())
            del bumped_pricer_for
            del model_for
            return (bumped_fair_value_for - self.calc_fair_value()) / bump_size
        elif self.fd_method == FiniteDifferenceMethod.BACKWARD:
            model_back = copy.deepcopy(self.model)
            model_back.bump_spot(-bump_size)
            bumped_pricer_back = self.create_pricer(self.contract, model_back, self.params)
            bumped_fair_value_back = (bumped_pricer_back.calc_fair_value())
            del bumped_pricer_back
            del model_back
            return (self.calc_fair_value() - bumped_fair_value_back) / bump_size
        elif self.fd_method == FiniteDifferenceMethod.CENTRAL:
            ...
            raise ValueError("Unknown finite difference method type!")
    ...
    def calc_gamma(self, method: GreekMethod) -> float:
        ...
        if self.bump_type == BumpType.RELATIVE:
            ...
            raise ValueError("Unknown bump type!")
        base_price = self.calc_fair_value()
        if self.fd_method == FiniteDifferenceMethod.CENTRAL:
            bumped_fair_values = list()
            for b in (bump_size, -bump_size):
                model = copy.deepcopy(self.model)
                model.bump_spot(b)
                ...
                del bumped_pricer
                del model
            return (bumped_fair_values[0] - 2 * base_price + bumped_fair_values[1]) / (bump_size ** 2)
        elif self.fd_method == FiniteDifferenceMethod.BACKWARD:
            model_back1 = copy.deepcopy(self.model)
            model_back1.bump_spot(-bump_size)
            price_back1 = self.create_pricer(self.contract, model_back1, self.params).calc_fair_value()
            model_back2 = copy.deepcopy(self.model)
            model_back2.bump_spot(-2 * bump_size)
            price_back2 = self.create_pricer(self.contract, model_back2, self.params).calc_fair_value()
            del model_back1, model_back2
            return (base_price - 2 * price_back1 + price_back2) / (bump_size ** 2)
        elif self.fd_method == FiniteDifferenceMethod.FORWARD:
            model_for1 = copy.deepcopy(self.model)
            model_for1.bump_spot(bump_size)
            price_for1 = self.create_pricer(self.contract, model_for1, self.params).calc_fair_value()
            model_for2 = copy.deepcopy(self.model)
            model_for2.bump_spot(2 * bump_size)
            price_for2 = self.create_pricer(self.contract, model_for2, self.params).calc_fair_value()
            del model_for1, model_for2
            return (price_for2 - 2 * price_for1 + base_price) / (bump_size ** 2)
