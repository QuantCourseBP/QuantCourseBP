...
from .model import *
import numpy as np
from .contract import Contract, EuropeanContract, AmericanContract
...
class MCMethod(NumericalMethod):
    def __init__(
        self, contract: Contract, model: MarketModel, params: MCParams
    ) -> None:
    ...
    def evolve_simulated_spot(
        ...
        if self.params.evolve_spot_method == MCNumMethod.EXACT:
            new_spot = spot_from * np.exp((rate - 0.5 * vol**2) * dt + vol * dW)
        elif self.params.evolve_spot_method == MCNumMethod.EULER:
            new_spot = spot_from + rate * spot_from * dt + vol * spot_from * dW
        elif self.params.evolve_spot_method == MCNumMethod.MILSTEIN:
            correction = 0.5 * vol**2 * spot_from * (dW**2 - dt)
            new_spot = spot_from + rate * spot_from * dt + vol * spot_from * dW + correction
        else:
...
class MCParams(Params):
    def __init__(
        self, seed: int = 1, num_of_path: int = 10000, tenor_frequency: int = 4,
