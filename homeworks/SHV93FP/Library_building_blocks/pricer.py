...
class EuropeanDigitalAnalyticPricer(Pricer):
    ...
    def calc_delta(self) -> float:
        ...
        d2 = EuropeanAnalyticPricer.calc_d2(spot / strike, vol, rate, time_to_expiry)
        if self.contract.derivative_type == PutCallFwd.CALL:
            greek = df /(spot * vol * np.sqrt(time_to_expiry)) * 1 /(2*np.pi) * np.exp(-1/2 * d2**2)
        elif self.contract.derivative_type == PutCallFwd.PUT:
            greek = -df /(spot * vol * np.sqrt(time_to_expiry)) * 1 /(2*np.pi) * np.exp(-1/2 * d2**2)
        else:
            self.contract.raise_incorrect_derivative_type_error()
    ...
    def calc_gamma(self) -> float:
        ...
        d2 = EuropeanAnalyticPricer.calc_d2(spot / strike, vol, rate, time_to_expiry)
        if self.contract.derivative_type == PutCallFwd.CALL:
            greek = -df /(spot**2 * vol**2 * time_to_expiry) * 1 /(2*np.pi) * np.exp(-1/2 * d2**2) * d1
        elif self.contract.derivative_type == PutCallFwd.PUT:
            greek = df /(spot**2 * vol**2 * time_to_expiry) * 1 /(2*np.pi) * np.exp(-1/2 * d2**2) * d1
        else:
            self.contract.raise_incorrect_derivative_type_error()
    ...
    def calc_vega(self) -> float:
        ...
        d2 = EuropeanAnalyticPricer.calc_d2(spot / strike, vol, rate, time_to_expiry)
        if self.contract.derivative_type == PutCallFwd.CALL:
            greek = -df /vol * 1 /(2*np.pi) * np.exp(-1/2 * d2**2) * d1
        elif self.contract.derivative_type == PutCallFwd.PUT:
            greek = df /vol * 1 /(2*np.pi) * np.exp(-1/2 * d2**2) * d1
        else:
            self.contract.raise_incorrect_derivative_type_error()
    ...
    def calc_theta(self) -> float:
        ...
        d2 = EuropeanAnalyticPricer.calc_d2(spot / strike, vol, rate, time_to_expiry)
        if self.contract.derivative_type == PutCallFwd.CALL:
            greek = df * (1 /(2*np.pi) * np.exp(-1/2 * d2**2) * 1/(2*time_to_expiry*vol*np.sqrt(time_to_expiry))
                            * (np.log(spot_over_strike) - (rate - vol**2 / 2) * time_to_expiry) + rate * norm.cdf(d2))
        elif self.contract.derivative_type == PutCallFwd.PUT:
            greek = df * (-1 /(2*np.pi) * np.exp(-1/2 * d2**2) * 1/(2*time_to_expiry*vol*np.sqrt(time_to_expiry))
                            * (np.log(spot_over_strike) - (rate - vol**2 / 2) * time_to_expiry) + rate * norm.cdf(-d2))
        else:
    ...
    def calc_rho(self) -> float:
        ...
        d2 = EuropeanAnalyticPricer.calc_d2(spot / strike, vol, rate, time_to_expiry)
        if self.contract.derivative_type == PutCallFwd.CALL:
            greek = df * (np.sqrt(time_to_expiry)/vol * 1 /(2*np.pi) * np.exp(-1/2 * d2**2) - time_to_expiry * norm.cdf(d2))
        elif self.contract.derivative_type == PutCallFwd.PUT:
            greek = df * (-np.sqrt(time_to_expiry)/vol * 1 /(2*np.pi) * np.exp(-1/2 * d2**2) - time_to_expiry * norm.cdf(-d2))
        else:
            self.contract.raise_incorrect_derivative_type_error()
