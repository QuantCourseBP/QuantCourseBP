...
class Pricer(ABC):
    ...
    finitemethod: FiniteMethod = FiniteMethod.CENTRAL
    bumpsizemethod: BumpSizeMethod = BumpSizeMethod.RELATIVE
    ...
    def calc_gamma(self, method: GreekMethod) -> float:
        ...
        #assignment: relative and abbsolute bump
        if method != GreekMethod.BUMP:
            self.raise_unsupported_greek_method_error(method, (GreekMethod.BUMP,))
        if self.bumpsizemethod == BumpSizeMethod.ABSOLUTE:
            bump_size = self.relative_bump_size
        ...
        bumped_fair_values = list()
        for b in (2 * bump_size, bump_size, 0, -bump_size, -2 * bump_size):   #for gamma we need 2h and -2h as well
            model = copy.deepcopy(self.model)
            model.bump_spot(b)
            bumped_pricer = self.create_pricer(self.contract, model, self.params)
            bumped_fair_values.append(bumped_pricer.calc_fair_value())
        ...
        if self.finitemethod == FiniteMethod.BACKWARD:   #changed the result for different type of finite method just like for delta
