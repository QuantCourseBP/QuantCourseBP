...
class GenericMCPricer(Pricer):
    ...
    ################### Implemennting the delta caluclation with the improved bump method
    def calc_delta(self, method: GreekMethod) -> float:
        if method != GreekMethod.BUMP:
            self.raise_unsupported_greek_method_error(method, (GreekMethod.BUMP,))
        ...
        bump_size = self.relative_bump_size * self.model.spot
        bump_relative = self.relative_bump_size
        ...
        for bump_mult in [1 + bump_relative, 1 - bump_relative]:

            path_payoff = np.empty(num_of_paths)

            for path in range(num_of_paths):
                bumped_spot_path = spot_paths[path, :] * bump_mult
                bumped_initial_spot = self.model.spot * bump_mult

                fixing_schedule = dict(zip(
                    [0] + contractual_timeline,
                    np.concatenate((np.array([bumped_initial_spot]), bumped_spot_path))
                ))

                path_payoff[path] = contract.payoff(fixing_schedule)

            fv = mean(path_payoff) * self.model.calc_df(contract.expiry)
            bumped_fair_values.append(fv)
...
class BarrierAnalyticPricer(Pricer):
    ...
    #################### implemennting bs_put just like for bs_call
    @staticmethod
    def bs_put(spot: float, strike: float, vol: float, rate: float, time_to_expiry: float, df: float) -> float:
        d1 = EuropeanAnalyticPricer.calc_d1(spot / strike, vol, rate, time_to_expiry)
        d2 = EuropeanAnalyticPricer.calc_d2(spot / strike, vol, rate, time_to_expiry)
    ...
    def calc_fair_value(self) -> float:
        ...
        if self.contract.derivative_type == PutCallFwd.CALL:
            if updown == UpDown.DOWN:
                ...
                if barrier > strike:
                    raise ValueError("DIC/DOC analytic formula is implemented only for B <= K.")   #I also added an error message here
        ...
        #################### implementing the pricer UIP AND UOP (B<=K)
        
        """else:
            self.raise_pricer_not_implemented_error()"""

        if self.contract.derivative_type == PutCallFwd.PUT:   #we need put know
            if updown == UpDown.UP:    #and UP 

                if barrier < strike:
                    raise ValueError("UIP/UOP analytic formula is implemented only for B >= K.")

                #calculating UIP
                price_uip = spot * (barrier / spot) ** (2 * rate / vol ** 2) * \
                    BarrierAnalyticPricer.bs_put(barrier / spot, strike / barrier, vol, rate, time_to_expiry, df)
                
                if inout == InOut.IN:
                    return direction * price_uip
                    
                if inout == InOut.OUT:
                    price_bs = BarrierAnalyticPricer.bs_put(spot, strike, vol, rate, time_to_expiry, df)
                    return direction * (price_bs - price_uip)
