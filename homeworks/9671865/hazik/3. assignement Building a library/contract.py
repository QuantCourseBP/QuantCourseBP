...
#5. heti házi - digitaal option
class EuropeanDigitalContract(Contract):
    def __init__(
        self,
        underlying: Stock,
        derivative_type: PutCallFwd,
    ...
    def payoff(self, spot: dict[float, float]) -> float:
        ...
        #csak ezt a résztt kellett változtatni, a kifizetést a sima európai opcióhoz képest
        if self.derivative_type == PutCallFwd.CALL:
            return self.direction * (1.0 if spot[t] > self.strike else 0.0)
        elif self.derivative_type == PutCallFwd.PUT:
            return self.direction * (1.0 if spot[t] < self.strike else 0.0)
        ####
