...
class TestDigital:
    def test_digital_analytic_pricer(self, underlying, strike_over_spot, expiry):
        ...
        result["const"] = dict()
        result["const"]['fv'] = df
        result["const"]['delta'] = 0
        result["const"]['gamma'] = 0
        result["const"]['vega'] = 0
        result["const"]['theta'] =  df * rate
        result["const"]['rho'] = -df*expiry
        result_put_call = {key: result['call'].get(key, 0) + result['put'].get(key, 0)
                           for key in set(result['call']) | set(result['put'])}
        assert result_put_call == pytest.approx(result['const'], abs=1e-6)
