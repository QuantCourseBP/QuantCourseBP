...
class Pricer(ABC):
    ...
    finite_diff_method: FiniteDiffMethod = FiniteDiffMethod.CENTRAL
    sizetype: AbsRel = AbsRel.RELATIVE
    ...
    def calc_delta(self, method: GreekMethod) -> float:
        ...
        if self.sizetype == AbsRel.RELATIVE:
            bump_size = self.bump_size * self.model.spot
        elif self.sizetype == AbsRel.ABSOLUTE:
        ...
        for b in needed:
            ...
            del bumped_pricer
            del model
        return (bumped_fair_values[0] - bumped_fair_values[1]) / (needed[0]-needed[1])
