...
class EuropeanDigitalAnalyticPricer(Pricer):
    ...
    def calc_delta(self) -> float:
        ...
        if self.contract.derivative_type == PutCallFwd.CALL:
            delta = df*norm.pdf(d2) / (spot*vol*np.sqrt(time_to_expiry))
        elif self.contract.derivative_type == PutCallFwd.PUT:
            delta = - (df*norm.pdf(d2)) / (spot*vol*np.sqrt(time_to_expiry))
        else:
            self.contract.raise_incorrect_derivative_type_error()
        return self.contract.direction * delta
    ...
    def calc_gamma(self) -> float:
        ...
        if self.contract.derivative_type == PutCallFwd.CALL:
            gamma = -(df*norm.pdf(d2)*d1) / ((spot**2)*(vol**2)*time_to_expiry)
        elif self.contract.derivative_type == PutCallFwd.PUT:
            gamma =  (df*norm.pdf(d2)*d1) / ((spot**2)*(vol**2)*time_to_expiry)
        else:
            self.contract.raise_incorrect_derivative_type_error()
    ...
    def calc_theta(self) -> float:
        ...
        if self.contract.derivative_type == PutCallFwd.CALL:
            theta = df*(norm.pdf(d2)/(2*time_to_expiry**(3/2)*vol)*(np.log(spot/strike)-(rate-(vol**2)/2)*time_to_expiry)+rate*norm.cdf(d2))
        elif self.contract.derivative_type == PutCallFwd.PUT:
            theta = df*(-norm.pdf(d2)/(2*time_to_expiry**(3/2)*vol)*(np.log(spot/strike)-(rate-(vol**2)/2)*time_to_expiry)+rate*norm.cdf(-d2))
        else:
            self.contract.raise_incorrect_derivative_type_error()
    ...
    def calc_rho(self) -> float:
        ...
        if self.contract.derivative_type == PutCallFwd.CALL:
            rho = df*(np.sqrt(time_to_expiry) * norm.pdf(d2)/vol - time_to_expiry*norm.cdf(d2))
        elif self.contract.derivative_type == PutCallFwd.PUT:
            rho = df*(-np.sqrt(time_to_expiry) * norm.pdf(d2)/vol - time_to_expiry*norm.cdf(-d2))
        else:
            self.contract.raise_incorrect_derivative_type_error()
