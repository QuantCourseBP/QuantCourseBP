...
class TestDigitalPutCallParity:
    def test_fair_value(self, underlying, strike_over_spot, expiry):
        params = Params()
        model = MarketModel(underlying)
        strike = strike_over_spot * model.spot

        contract_call = EuropeanDigitalContract(underlying, PutCallFwd.CALL, LongShort.LONG, strike, expiry)
        pricer_call = EuropeanDigitalAnalyticPricer(contract_call, model, params)

        contract_put = EuropeanDigitalContract(underlying, PutCallFwd.PUT, LongShort.LONG, strike, expiry)
        pricer_put = EuropeanDigitalAnalyticPricer(contract_put, model, params)

        fv_call = pricer_call.calc_fair_value()
        fv_put = pricer_put.calc_fair_value()
        df = model.calc_df(expiry)

        assert fv_call + fv_put == pytest.approx(df, abs=1e-8)