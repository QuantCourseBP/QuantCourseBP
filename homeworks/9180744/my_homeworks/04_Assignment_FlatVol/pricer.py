...
class Pricer(ABC):
    ...
    bumpsize_method: BumpSizeMethod = BumpSizeMethod.RELATIVE
    ...
    def calc_delta(self, method: GreekMethod) -> float:
        ...
        if self.finite_method == FiniteMethod.BACKWARD: #backward: (f(S) - f(S-h)) / h
            result = (bumped_fair_values[1] - bumped_fair_values[2])/bump_size
        elif self.finite_method == FiniteMethod.FORWARD: #forward: (f(S+h) - f(S)) / h
            result = (bumped_fair_values[0] - bumped_fair_values[1])/bump_size
        else: #central: (f(S+h) - f(S-h)) / h
            result = (bumped_fair_values[0] - bumped_fair_values[2]) / (2 * bump_size)
    ...
    def calc_gamma(self, method: GreekMethod) -> float:
        ...
        bumped_fair_values = list()
        for b in (2*bump_size, bump_size, 0, -bump_size, -2*bump_size):
            model = copy.deepcopy(self.model)
            model.bump_spot(b)
            bumped_pricer = self.create_pricer(self.contract, model, self.params)
            bumped_fair_values.append(bumped_pricer.calc_fair_value())
        ...
        if self.finite_method == FiniteMethod.BACKWARD: #backward: (f(S) - 2f(S-h) + f(S-2h)) / (h^2)
            result = (bumped_fair_values[2] - 2 * bumped_fair_values[3] + bumped_fair_values[4]) / (bump_size ** 2)
        elif self.finite_method == FiniteMethod.FORWARD: #forward: (f(S+2h) - 2f(S+h) + f(S)) / (h^2)
            result = (bumped_fair_values[0] - 2 * bumped_fair_values[1] + bumped_fair_values[2]) / (bump_size ** 2)
        else: #central: (f(S+h) - 2f(S) + f(S-h)) / (h^2)
            result = (bumped_fair_values[1] - 2 * bumped_fair_values[2] + bumped_fair_values[3]) / (bump_size ** 2)
