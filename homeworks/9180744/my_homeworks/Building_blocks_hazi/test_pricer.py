...
@pytest.mark.parametrize('strike_over_spot', np.arange(0.5, 2.0, 0.5))
@pytest.mark.parametrize('expiry', np.arange(0.5, 2.5, 0.5))
class TestDigitalPutCallParity:
    def test_analytic_pricer(self, underlying, strike_over_spot, expiry):
        pricer = dict()
        result = dict()
        params = Params()
        ...
        payout = 1
        contract_call = EuropeanDigitalContract(underlying, PutCallFwd.CALL, LongShort.LONG, strike, expiry, payout)
        pricer['call'] = EuropeanDigitalAnalyticPricer(contract_call, model, params)
        contract_put = EuropeanDigitalContract(underlying, PutCallFwd.PUT, LongShort.LONG, strike, expiry, payout)
        pricer['put'] = EuropeanDigitalAnalyticPricer(contract_put, model, params)
        for deriv_type in pricer.keys():
            result[deriv_type] = dict()
            result[deriv_type]['fv'] = pricer[deriv_type].calc_fair_value()
            result[deriv_type]['delta'] = pricer[deriv_type].calc_delta()
            ...
            result[deriv_type]['theta'] = pricer[deriv_type].calc_theta()
            result[deriv_type]['rho'] = pricer[deriv_type].calc_rho()
        result_digital_put_call = {key: result['call'].get(key, 0) + result['put'].get(key, 0)
                                   for key in set(result['call']) | set(result['put'])}

        rate = model.risk_free_rate
        time_to_expiry = expiry
        df = model.calc_df(time_to_expiry)
        expected_bond = dict()
        expected_bond['fv'] = payout * df
        expected_bond['delta'] = 0.0
        expected_bond['gamma'] = 0.0
        expected_bond['vega'] = 0.0
        expected_bond['theta'] = rate * payout * df
        expected_bond['rho'] = -time_to_expiry * payout * df
        assert result_digital_put_call == pytest.approx(expected_bond, abs=1e-6)