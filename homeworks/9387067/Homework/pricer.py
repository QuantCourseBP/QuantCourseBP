...
class EuropeanDigitalAnalyticPricer(Pricer):
    ...
    @staticmethod
    def calc_d2(spot_over_strike: float, vol: float, rate: float, time_to_expiry: float) -> float:
        d1 = EuropeanDigitalAnalyticPricer.calc_d1(spot_over_strike, vol, rate, time_to_expiry)
        return d1 - vol * np.sqrt(time_to_expiry)
    ...
    def calc_fair_value(self) -> float:
        ...
        strike = self.contract.strike
        expiry = self.contract.expiry
        cash_payoff = self.contract.cash_payoff
        time_to_expiry = expiry - self.valuation_time
        spot = self.model.spot
        vol = self.model.vol
        rate = self.model.risk_free_rate
        df = self.model.calc_df(time_to_expiry)
        d2 = EuropeanDigitalAnalyticPricer.calc_d2(spot / strike, vol, rate, time_to_expiry)
        if self.contract.derivative_type == PutCallFwd.CALL:
            return direction * (cash_payoff * df * norm.cdf(d2))
        elif self.contract.derivative_type == PutCallFwd.PUT:
            return direction * (cash_payoff * df * norm.cdf(-d2))
        else:
            self.contract.raise_incorrect_derivative_type_error()
    ...
    def calc_delta(self) -> float:
        ...
        d2 = EuropeanDigitalAnalyticPricer.calc_d2(spot / strike, vol, rate, time_to_expiry)
        if self.contract.derivative_type == PutCallFwd.CALL:
            greek = cash_payoff * df * norm.pdf(d2) / (spot * vol * np.sqrt(time_to_expiry))
        elif self.contract.derivative_type == PutCallFwd.PUT:
            greek = -1.0 * cash_payoff * df * norm.pdf(d2) / (spot * vol * np.sqrt(time_to_expiry))
        else:
            self.contract.raise_incorrect_derivative_type_error()
        return self.contract.direction * greek
    ...
    def calc_gamma(self) -> float:
        ...
        vol = self.model.vol
        rate = self.model.risk_free_rate
        d1 = EuropeanDigitalAnalyticPricer.calc_d1(spot / strike, vol, rate, time_to_expiry)
        d2 = EuropeanDigitalAnalyticPricer.calc_d2(spot / strike, vol, rate, time_to_expiry)
        if self.contract.derivative_type == PutCallFwd.CALL:
            greek = -1.0 * cash_payoff * df * norm.pdf(d2) * d1 / (spot**2 * vol**2 * time_to_expiry)
        elif self.contract.derivative_type == PutCallFwd.PUT:
            greek = cash_payoff * df * norm.pdf(d2) * d1 / (spot**2 * vol**2 * time_to_expiry)
        else:
            self.contract.raise_incorrect_derivative_type_error()
        return self.contract.direction * greek
    ...
    def calc_vega(self) -> float:
        ...
        d2 = EuropeanDigitalAnalyticPricer.calc_d2(spot / strike, vol, rate, time_to_expiry)
        if self.contract.derivative_type == PutCallFwd.CALL:
            greek = -1.0 * cash_payoff * df * norm.pdf(d2) * d1 / vol
        elif self.contract.derivative_type == PutCallFwd.PUT:
            greek = cash_payoff * df * norm.pdf(d2) * d1 / vol
        else:
            self.contract.raise_incorrect_derivative_type_error()
        return self.contract.direction * greek
    ...
    def calc_theta(self) -> float:
        ...
        d2 = EuropeanDigitalAnalyticPricer.calc_d2(spot / strike, vol, rate, time_to_expiry)
        if self.contract.derivative_type == PutCallFwd.CALL:
            greek = cash_payoff * df * (norm.pdf(d2) / (2 * time_to_expiry * vol * np.sqrt(time_to_expiry)) * (np.log(spot / strike) - (rate - vol**2 / 2) * time_to_expiry) + rate * norm.cdf(d2))
        elif self.contract.derivative_type == PutCallFwd.PUT:
            greek = cash_payoff * df * (-1.0 * norm.pdf(d2) / (2 * time_to_expiry * vol * np.sqrt(time_to_expiry)) * (np.log(spot / strike) - (rate - vol**2 / 2) * time_to_expiry) + rate * norm.cdf(-d2))
        else:
            self.contract.raise_incorrect_derivative_type_error()
        return self.contract.direction * greek
    ...
    def calc_rho(self) -> float:
        ...
        d2 = EuropeanDigitalAnalyticPricer.calc_d2(spot / strike, vol, rate, time_to_expiry)
        if self.contract.derivative_type == PutCallFwd.CALL:
            greek = cash_payoff * df * (np.sqrt(time_to_expiry) / vol * norm.pdf(d2) - time_to_expiry * norm.cdf(d2))
        elif self.contract.derivative_type == PutCallFwd.PUT:
            greek = cash_payoff * df * (-1.0 * np.sqrt(time_to_expiry) / vol * norm.pdf(d2) - time_to_expiry * norm.cdf(-d2))
        else:
            self.contract.raise_incorrect_derivative_type_error()
        return self.contract.direction * greek
