    ...
    def calc_fair_value(self) -> float:
        ...
        if self.contract.derivative_type == PutCallFwd.CALL:
            if updown == UpDown.DOWN:
                ...
                if inout == InOut.OUT:
                    price_bs = direction * BarrierAnalyticPricer.bs_call(spot, strike, vol, rate, time_to_expiry, df)
                    price_dic = direction * spot * (barrier / spot) ** (2 * rate / vol ** 2) * \
                        BarrierAnalyticPricer.bs_call(barrier / spot, strike / barrier, vol, rate, time_to_expiry, df)
                    return price_bs - price_dic
        if self.contract.derivative_type == PutCallFwd.PUT:
            if updown == UpDown.DOWN:
                if inout == InOut.IN:
                    return direction * spot * (barrier / spot) ** (2 * rate / vol **2) * \
                        BarrierAnalyticPricer.bs_put(barrier / spot, strike / barrier, vol, rate, time_to_expiry, df)
                if inout == InOut.OUT:
                    price_bs = direction * BarrierAnalyticPricer.bs_put(spot, strike, vol, rate, time_to_expiry, df)
                    price_dic = direction * spot * (barrier / spot) ** (2 * rate / vol ** 2) * \
                        BarrierAnalyticPricer.bs_put(barrier / spot, strike / barrier, vol, rate, time_to_expiry, df)
                    return price_bs - price_dic
        else:
