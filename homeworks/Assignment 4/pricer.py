...
class Pricer(ABC):
    ...
    finite_method: FiniteMethod = FiniteMethod.CENTRAL
    bump_size_method: BumpSizeMethod = BumpSizeMethod.RELATIVE
    ...
    def _get_bump_size(self) -> float:
        """Return the absolute bump size in spot units based on bump_size_method."""
        if self.bump_size_method == BumpSizeMethod.RELATIVE:
            return self.relative_bump_size * self.model.spot
        else:  # ABSOLUTE
            return self.relative_bump_size

    def _bumped_fv(self, bump: float) -> float:
        """Return fair value with spot bumped by an absolute amount."""
        model = copy.deepcopy(self.model)
        model.bump_spot(bump)
        bumped_pricer = self.create_pricer(self.contract, model, self.params)
        fv = bumped_pricer.calc_fair_value()
        del bumped_pricer
        del model
        return fv
    ...
    def calc_delta(self, method: GreekMethod) -> float:
        ...
        h = self._get_bump_size()
        if self.finite_method == FiniteMethod.FORWARD:
            return (self._bumped_fv(h) - self.calc_fair_value()) / h
        elif self.finite_method == FiniteMethod.BACKWARD:
            return (self.calc_fair_value() - self._bumped_fv(-h)) / h
        elif self.finite_method == FiniteMethod.CENTRAL:
            return (self._bumped_fv(h) - self._bumped_fv(-h)) / (2 * h)

    def calc_gamma(self, method: GreekMethod) -> float:
        if method != GreekMethod.BUMP:
            self.raise_unsupported_greek_method_error(method, (GreekMethod.BUMP,))
        h = self._get_bump_size()
        fv_0 = self.calc_fair_value()
        if self.finite_method == FiniteMethod.CENTRAL:
            return (self._bumped_fv(h) - 2 * fv_0 + self._bumped_fv(-h)) / (h ** 2)
        elif self.finite_method == FiniteMethod.FORWARD:
            # One-sided forward: f(x+2h) - 2f(x+h) + f(x)
            return (self._bumped_fv(2 * h) - 2 * self._bumped_fv(h) + fv_0) / (h ** 2)
        elif self.finite_method == FiniteMethod.BACKWARD:
            # One-sided backward: f(x) - 2f(x-h) + f(x-2h)
            return (fv_0 - 2 * self._bumped_fv(-h) + self._bumped_fv(-2 * h)) / (h ** 2)
        else:
            raise ValueError(f'Unknown FiniteMethod: {self.finite_method}')
