...
class Pricer(ABC):
    ...
    def calc_gamma(self, method: GreekMethod) -> float:
        ...
        bump_size = self.relative_bump_size * self.model.spot
        bumped_fair_values = list()
        for b in (2*bump_size, bump_size, 0, -bump_size, -2*bump_size): # change the loop, model.bump spot(0) = self.calc_fair_value()
            model = copy.deepcopy(self.model)
            model.bump_spot(b)
            bumped_pricer = self.create_pricer(self.contract, model, self.params)
            bumped_fair_values.append(bumped_pricer.calc_fair_value())
            ...
            del model
        if self.finitemethod == FiniteMethod.BACKWARD:
            result =  (bumped_fair_values[2] - 2 * bumped_fair_values[3] + bumped_fair_values[4]) / (bump_size ** 2) # b: f(x)- 2f(x-h)+f(x-2h)
        elif self.finitemethod == FiniteMethod.FORWARD:
            result = (bumped_fair_values[2] - 2 * bumped_fair_values[1] + bumped_fair_values[0]) / (bump_size ** 2) # f: f(x+h)- 2f(x)+f(x-h)
        else:
            result = (bumped_fair_values[1] - 2 * bumped_fair_values[2] + bumped_fair_values[3]) / (bump_size ** 2) # c: f(x+h)- 2f(x)+f(x-h)
